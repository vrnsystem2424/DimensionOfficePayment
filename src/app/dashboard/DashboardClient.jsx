...
import Payment   from '../Office/Payment/page';
// ✅ NEW - Advance Form
import AdvanceForm from '../Office/Advanceform/page';
...
const USER_TABS = {
  ...
  VIJAY: [
    ...
    'billentry',
    'payment',
    'advanceform',        // ✅ Added (remove if VIJAY shouldn't see it)
    'reconciliation',
    'form',
    'bankcharges',
  ],
  RICHA: [
    ...
    'billentry',
    'payment',
    'advanceform',        // ✅ Added (remove if RICHA shouldn't see it)
  ],
};

// ✅ Added advanceform to OFFICE_TABS
const OFFICE_TABS = ['approve1', 'billentry', 'payment', 'advanceform'];
const PAYMENT_TABS = ['reconciliation', 'form', 'actualpayment', 'transferbank', 'bankcharges'];
...
const TAB_LABELS = {
  ...
  billentry:      'Bill Entry',
  payment:        'Payment',
  advanceform:    'Advance Form',   // ✅ Added
  reconciliation: 'Reconciliation',
  form:           'Form',
  actualpayment:  'Actual Payment In',
  transferbank:   'Transfer Bank',
  bankcharges:    'Bank Charges & Interest',
};
...
export default function Dashboard() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'billentry':      return <BillEntry user={user} />;
      case 'payment':        return <Payment user={user} />;
      case 'advanceform':    return <AdvanceForm user={user} />;   // ✅ Added
      case 'reconciliation': return <Reconciliation user={user} />;
      case 'form':           return <Form user={user} />;
      case 'actualpayment':  return <ActualPaymentIn user={user} />;
      case 'transferbank':   return <TransferBank user={user} />;
      case 'bankcharges':    return <BankChargesInterestForm user={user} />;
      default:               return <Summary user={user} />;
    }
  };
  ...
  const isReconciliationTab = PAYMENT_TABS.includes(activeTab);
        ...
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden lg:flex flex-1 justify-center items-center space-x-6">
              ...
              {/* ✅ Office Dropdown - now includes Advance Form */}
              {showOffice && (
                <div className="relative">
                  <button
                    onClick={(e) => {
                    ...
                    className={`flex items-center space-x-1.5 px-5 py-2 rounded-lg font-medium text-base transition-colors ${
                      OFFICE_TABS.includes(activeTab)
                        ? 'bg-blue-100 text-blue-800'
                        : 'text-gray-700 hover:bg-gray-100'
                    }`}
                  >
              ...
              {showPayment && (
                <div className="relative">
                  <button
                    ...
                    className={`flex items-center space-x-1.5 px-5 py-2 rounded-lg font-medium text-base transition-colors ${
                      PAYMENT_TABS.includes(activeTab)
        ...
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute top-0 left-0 h-full w-64 bg-white shadow-2xl flex flex-col">
            ...
            <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
              ...
              {/* ✅ Office section - includes Advance Form */}
              {visibleOfficeTabs.length > 0 && (
                <div className="mb-4">
            ...
            <div className="p-4 border-t">
              <button
                onClick={handleLogout}
                className="w-full bg-red-600 text-white py-2 rounded-lg text-sm font-medium flex items-center justify-center space-x-2"
              >
                <LogOut className="w-4 h-4" />
                <span>Logout</span>
              </button>
