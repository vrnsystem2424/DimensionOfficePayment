...
const DimPayment = ({ user }) => {
  ...
  const toggleBill = (uid) => {
    setSelectedBills((prev) => {
      if (prev.includes(uid)) {
        setPaidAmounts((p) => { const n = { ...p }; delete n[uid]; return n; });
        setTdsAmounts((p) => { const n = { ...p }; delete n[uid]; return n; });
        // ✅ Remove professional tax on deselect
      ...
      setPaidAmounts((p) => ({ ...p, [uid]: "" }));
      setTdsAmounts((p) => ({ ...p, [uid]: "" }));
      // ✅ Init professional tax on select
  ...
  const selectAllBills = () => {
    const allUids = filteredBills.map((b) => b.uid);
    setSelectedBills(allUids);
    const newPaid = {}, newTds = {}, newPT = {}, newRound = {}, newRemarks = {};
    allUids.forEach((uid) => {
      newPaid[uid] = paidAmounts[uid] ?? "";
      newTds[uid] = tdsAmounts[uid] ?? "";
      // ✅ Init professional tax for all
      newPT[uid] = professionalTaxAmounts[uid] ?? "";
      newRound[uid] = roundups[uid] ?? 0;
      newRemarks[uid] = remarks[uid] ?? "";
    });
    setPaidAmounts(newPaid);
    setTdsAmounts(newTds);
    setProfessionalTaxAmounts(newPT);
    setRoundups(newRound);
    setRemarks(newRemarks);
  };
  ...
  // ✅ Grand total: Paid - TDS - Professional Tax + Roundup
  const grandTotal = selectedBills.reduce((sum, uid) => {
    const paid = Number(paidAmounts[uid] || 0);
    const tds = Number(tdsAmounts[uid] || 0);
  ...
  const handleSubmit = async () => {
    ...
    try {
      for (const uid of selectedBills) {
        ...
        const pt = Number(professionalTaxAmounts[uid] || 0);
        const roundup = Number(roundups[uid] || 0);
        // ✅ Net Payable = Paid - TDS - Professional Tax + Roundup
      ...
      {showBills && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {filteredBills.length > 0 && (
                <button onClick={selectedBills.length === filteredBills.length ? deselectAllBills : selectAllBills} style={{ background: "none", border: "1px solid #c7d2fe", borderRadius: 8, padding: "7px 14px", cursor: "pointer", color: "#4f46e5", fontWeight: 600, fontSize: 12, fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 }}>
                  <FaCheckCircle style={{ fontSize: 11 }} />
                  {selectedBills.length === filteredBills.length ? "Deselect All" : "Select All"}
                </button>
              )}
              <button onClick={handleClearFilters} style={{ background: "none", border: "1px solid #e2e8f0", borderRadius: 8, padding: "7px 14px", cursor: "pointer", color: "#64748b", fontWeight: 600, fontSize: 13, display: "flex", alignItems: "center", gap: 6, fontFamily: "inherit" }}>
                <FaTimes style={{ fontSize: 11 }} /> Clear
              </button>
            </div>
          </div>
            ...
            filteredBills.map((bill) => {
              ...
              return (
                <div key={bill.uid} style={{
                  ...
                  {bill.Bill_Photo && (
                    <div style={{ marginTop: 14 }}>
                      <a href={bill.Bill_Photo} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: "#6366f1", fontWeight: 600, textDecoration: "none", border: "1px solid #6366f130", borderRadius: 6, padding: "5px 12px", backgroundColor: "#6366f108" }}>
                        📎 View Bill Photo
                      </a>
                    </div>
                  )}

                  {/* ✅ Payment Entry Section — 6 columns ab */}
                  {isSelected && (
                    <div style={{ marginTop: 20, padding: "18px 20px", backgroundColor: "#f8f9ff", borderRadius: 12, border: "1px solid #e0e7ff" }}>
                      ...
                      {/* ✅ 6 columns: Paid | TDS | Prof Tax | Round Off | Net Payable | Remark */}
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr 1fr", gap: 16 }}>
                        ...
                        <div>
                          <label style={{ fontSize: 11, fontWeight: 700, color: "#64748b", display: "block", marginBottom: 6, letterSpacing: "0.07em" }}>
                            TDS ₹{" "}
                            <span style={{ marginLeft: 4, fontSize: 9, fontWeight: 600, color: "#94a3b8", backgroundColor: "#f1f5f9", borderRadius: 4, padding: "1px 5px" }}>OPTIONAL</span>
                          </label>
                          <input
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const val = e.target.value;
                              if (val === "") {
                                setProfessionalTaxAmounts((p) => ({ ...p, [bill.uid]: "" }));
                                return;
                              }
                              let v = Number(val);
                              const paid = Number(paidAmounts[bill.uid] || 0);
                              ...
                              if (v > maxPT) {
                                alert(`Professional Tax (₹${v}) (Paid - TDS) = ₹${fmt(maxPT)} से ज़्यादा नहीं हो सकता`);
                                v = maxPT;
                              }
                            ...
                            style={{
                              ...
                              boxSizing: "border-box", fontFamily: "inherit", backgroundColor: "#faf5ff"
                            }}
                          />
                        </div>
          ...
          {selectedBills.length > 0 && (
            <div style={{
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: "#4f46e5" }}>
                ...
                {/* ✅ Professional Tax total in grand total */}
