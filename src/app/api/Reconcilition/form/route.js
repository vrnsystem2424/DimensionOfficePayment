...
//     if (action === 'dropdown-data') {
//       // ✅ Parallel fetch: N4:P for main dropdowns + L4:L for capital projects
//       const [mainResponse, capitalProjResponse] = await Promise.all([
...
//       const capitalProjRows = capitalProjResponse.data.values || [];
...
//       // ✅ Capital projects from column L
//       const capitalProjectSet = new Set();
//       capitalProjRows.forEach(row => {
//         const val = row[0]?.toString().trim();
//         if (val) capitalProjectSet.add(val);
//       });
...
//         accounts: [...accountSet].sort(),
//         capitalMovements: [...movementSet].sort(),
//         capitalProjects: [...capitalProjectSet].sort(), // ✅ NEW
//       });
//     }
...
//     if (action === 'Captial-A/C') {
//       const {
//         ProjectName = '',        // ✅ NEW — J column mein jayega
//         Capital_Movment,
//         Received_Account,
//         Amount,
//         PAYMENT_MODE,
...
//         PAYMENT_MODE,       // F
//         PAYMENT_DETAILS,    // G
//         formatDateToDDMMYYYY(PAYMENT_DATE), // H
//         Remark,             // I
//         ProjectName,        // J ✅ NEW
//       ];
...
//       await sheets.spreadsheets.values.append({
//         spreadsheetId,
//         range: 'Capital_Movement_Form!A:J',  // ✅ A:J (was A:I)
//         valueInputOption: 'USER_ENTERED',
//         requestBody: { values: [row] },
//       });
...
export async function GET(request) {
  try {
    ...
    if (action === 'dropdown-data') {
      // ✅ Parallel fetch: N4:P + L4:L + M4:M
      const [mainResponse, capitalProjResponse, clientProjResponse] = await Promise.all([
        sheets.spreadsheets.values.get({
          spreadsheetId,
          range: 'Project_Data!N4:P',
        }),
        ...
        sheets.spreadsheets.values.get({
          spreadsheetId,
          range: 'Project_Data!M4:M',
        }),
      ]);

      const rows = mainResponse.data.values || [];
      const capitalProjRows = capitalProjResponse.data.values || [];
      const clientProjRows = clientProjResponse.data.values || [];
      ...
      // ✅ Client projects from column M
      const clientProjectSet = new Set();
      clientProjRows.forEach(row => {
        const val = row[0]?.toString().trim();
        if (val) clientProjectSet.add(val);
      });
      ...
      return NextResponse.json({
        ...
        accounts: [...accountSet].sort(),
        capitalMovements: [...movementSet].sort(),
        capitalProjects: [...capitalProjectSet].sort(),
        clientProjects: [...clientProjectSet].sort(), // ✅ M column
      });
    }
...
export async function POST(request) {
  try {
    ...
    if (action === 'add-payment') {
      const {
        SiteName,
        ClientProject = '',  // ✅ M column data → D column
        Amount,
        CGST = 0,
        SGST = 0,
        NetAmount = 0,
        RccCreditAccountName = '',  // ✅ fixed name
        PaymentMode = '',
        ChequeNo = '',
        ChequeDate = '',
        ChequePhoto = '',
      ...
      const row = [
        ...
        UID,                     // B
        SiteName,                // C
        ClientProject || '',     // D ✅ M column data
        Amount || 0,             // E
        CGST || 0,               // F
        SGST || 0,               // G
        NetAmount || 0,          // H
        RccCreditAccountName || '', // I
        PaymentMode || '',       // J
        ChequeNo || '',          // K
        formatDateToDDMMYYYY(ChequeDate), // L
        chequePhotoUrl,          // M
      ];

      await sheets.spreadsheets.values.append({
        spreadsheetId,
        range: 'Client_Payment_In_FMS!A:M',
        valueInputOption: 'USER_ENTERED',
        insertDataOption: 'INSERT_ROWS',
        requestBody: { values: [row] },
      });
